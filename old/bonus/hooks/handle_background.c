...
/*   Updated: 2024/08/28 18:42:10 by mbirou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cd_update_frame(t_info *info)
{
	...
	gettimeofday(&time, NULL);
	if (secs == time.tv_sec && info->current_display == 1)
	{
		...
		mlx_string_put(info->mlx_info->mlx, info->mlx_info->mlx_win, 1, 10, cd_invert_color(cd_get_color(info->mlx_info->main_frame, 6, 6)), fps);
		free(fps);
	}
	return (0);
}
