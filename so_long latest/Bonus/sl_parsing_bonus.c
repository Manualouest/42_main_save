...
/*   Updated: 2024/04/06 14:33:12 by mbirou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (m_inf->map[++xy.y] != 0)
	{
		...
		while (m_inf->map[xy.y][++xy.x] != 0)
		{
			if ((xy.y % (m_inf->size.y - 1) == 0
				...
				return (2);
			if (sl_check_tile_utile(m_inf, &parse_util, xy, pxy) != 1)
				return (3);
		}
...
void	sl_path_check_utils(t_map_info *map_info, int x, int y)
{
	...
	if (map_info->map_copy[y][x] != 'E' && map_info->map_copy[y][x] != 'H')
	{
		if (map_info->map_copy[y][x] > 0)
			map_info->map_copy[y][x] = '1';
		if (y > 1 && map_cpy[y - 1][x] != '1' && map_cpy[y - 1][x] > 0)
			sl_path_check_utils(map_info, x, y - 1);
		if (x < width - 1 && map_cpy[y][x + 1] != '1' && map_cpy[y][x + 1] > 0)
			sl_path_check_utils(map_info, x + 1, y);
		if (y < height - 1 && map_cpy[y + 1][x] != '1' && map_cpy[y + 1][x] > 0)
			sl_path_check_utils(map_info, x, y + 1);
		if (x > 1 && map_cpy[y][x - 1] != '1' && map_cpy[y][x - 1] > 0)
			sl_path_check_utils(map_info, x - 1, y);
	}
	else
		if (map_info->map_copy[y][x] > 0)
...
int	sl_parse_main(char *map_file, t_map_info *map_info)
{
	...
	map_full = NULL;
	if (check_file_type(map_file) == 0)
		return (sl_parsing_error(8, map_info, NULL, 0));
	sl_get_map(map_file, &map_full, &tpint);
	close(tpint);
	if (map_full == NULL)
		return (sl_parsing_error(1, map_info, &map_full, 0));
	if (sl_has_empty_lines(map_full) == 1)
		return (sl_parsing_error(7, map_info, &map_full, 0));
	...
	tpint = sl_check_tiles(map_info, &xy);
	if (tpint != 1)
