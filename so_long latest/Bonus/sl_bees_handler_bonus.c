...
/*   Updated: 2024/04/08 21:03:03 by mbirou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sl_move_bee(t_map_info *map_info, t_bees *bee)
{
	...
	{
		num = -1;
		rand = (map_info->gifs->exit_frame * bee->bee_frame) % 2;
		if (rand == 0 && bee->bee_pos.x != bee->target.x)
			num = sl_move_bee_on_x(bee, map_info);
		if (rand == 0 && num == 0)
			sl_move_bee_on_y(bee, map_info);
		if (rand == 0 && num == 0)
			sl_move_bee_on_x(bee, map_info);
		if (num == -1 && bee->bee_pos.y != bee->target.y)
			num = sl_move_bee_on_y(bee, map_info);
		if (num == 0 || num == -1)
			sl_move_bee_on_x(bee, map_info);
		if (num == 0)
			sl_move_bee_on_y(bee, map_info);
