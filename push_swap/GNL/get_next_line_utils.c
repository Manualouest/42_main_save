...
/*   Updated: 2024/01/24 20:06:41 by mbirou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_until_n(char *str, int free_chc)
{
	...
	i = 0;
	nlen = ft_len_str_n_gnl(str, BUFFER_SIZE, 1);
	if (!nlen)
		return (NULL);
	if (nlen == -1)
		return (str);
...
char	*ft_strlcat_gnl(char *dest, char *src, int free_chc)
{
	...
	i = 0;
	len = ft_len_str_n_gnl(dest, 0, 0) + ft_len_str_n_gnl(src, 0, 0);
	tp = (char *)ft_calloc_gnl(len, 1);
	if (!tp)
		return (NULL);
	len = 0;
	while (dest && dest[i] != 0)
